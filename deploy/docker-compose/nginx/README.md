...
> MuseFlow 应用的入口配置（HTTPS 终止 + 反代 Traefik）属于 K3s 部署方式的一部分，
> 位于 `deploy/k8s/edge/nginx/museflow.com.conf`，不在本目录。
...
- 浏览器通过绝对路径 `/errors/error.css` 加载样式，由 `museflow.com.conf` 的
  `location ^~ /errors/` 提供。
- MuseFlow 应用站（`deploy/k8s/edge/nginx/museflow.com.conf`）同样通过
  `location ^~ /errors/` 复用 `errors/` 下的 `502/503/504` 页面。
