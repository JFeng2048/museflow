...
// （auth_router.go / user_router.go / admin_router.go / model_router.go），
...
func NewHandlers(cfg *config.Config, userClient *client.UserClient, modelClient *client.ModelClient) *Handlers {
	return &Handlers{
		...
		Model:      handler.NewModelHandler(modelClient),
	}
}
...
// 需要登录的接口会在各域路由中按需挂载。
func Register(r *gin.RouterGroup, cfg *config.Config, userClient *client.UserClient, modelClient *client.ModelClient) {
	h := NewHandlers(cfg, userClient, modelClient)
	...
	registerModelRoutes(r, h, userClient, auth)
}
