...
- **Startup seeding (`internal/bootstrap`)**: on boot, `cmd/server` heals the whole RBAC reference set — the system roles (`super_admin` / `admin` / `user`), the 17 permission definitions and the default role-permission mappings (`super_admin` gets all, `user` gets the 8 authoring ones) — and creates the admin account from `USER_ADMIN_EMAIL` / `USER_ADMIN_PASSWORD`, granting it `super_admin`. An **empty database can therefore boot straight away** with no manual reference-data import. It is idempotent: existing roles, permissions and accounts are skipped; an existing password is **never** overwritten (set `USER_ADMIN_RESET_PASSWORD=true` for one restart to reset it, then flip it back); role permissions edited in the admin UI are **never** overwritten (defaults are only written when a role has none). A seeding failure is logged without blocking startup.
...
- **RBAC cache + `super_admin` wildcard**: permission lookups are cached in Redis with a TTL equal to the refresh-token lifetime (7 days) to cut DB load. `super_admin` is **not** granted permission-by-permission — the cache holds a single wildcard marker (`*`) and `CheckPermission` short-circuits on it, so a new permission code needs no re-granting and there is no window where a cached list is missing one. When permissions are queried through the API the wildcard is expanded into all codes, so frontend menus and buttons need no special handling.
