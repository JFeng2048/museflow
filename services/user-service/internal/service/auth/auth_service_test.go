...
func newTestService() (*AuthService, *fakeTokenStore) {
	...
	// bcrypt 使用最小成本加速测试（rbac / audit 传 nil：测试聚焦于认证主流程）
	svc := NewAuthService(newFakeUserRepo(), store, tm, nil, nil, bcrypt.MinCost)
	return svc, store
}
...
func TestLoginLocksAccountAfterMaxFailures(t *testing.T) {
	...
	svc := NewAuthService(repo, store, tm, nil, nil, bcrypt.MinCost)
	ctx := context.Background()
